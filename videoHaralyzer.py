...
def process_har_file(har_file_path, output_excel_path):
    ...
    for page in har_parser.pages:
        for entry in page.entries:
            ...
            if '.mp4' in entry['request']['url'] or '.MOV' in entry['request']['url']:
    ...
    # 데이터프레임 생성
    df = pd.DataFrame(movie_requests)

    # 결과 출력
    print(f"Total MP4/MOV requests found in {har_file_path}: {len(movie_requests)}")
    print(df)
...
def process_files_in_directory(directory_path):
    ...
    for file_name in files:
        if file_name.endswith('.har'):  # HAR 파일만 처리
            ...
            output_excel_path = os.path.join(directory_path, file_name.replace('.har', '.xlsx'))
